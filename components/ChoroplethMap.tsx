...
const ChoroplethMap: React.FC = () => {
  ...
  const choroplethMethod = useSelector((state: RootState) => state.root.choroplethMethod);
  const numberOfBreaks = useSelector((state: RootState) => state.root.numberOfBreaks);
  const selectedChoroplethVariable = useSelector(
    (state: RootState) => state.root.selectedChoroplethVariable
  );
  const data = useSelector((state: RootState) => state.root.file.rawFileData);
  const layers = useSelector((state: RootState) => state.keplerGl[mapId]?.visState?.layers);

  const jenksIdx = useSelector((state: RootState) =>
  ...
  const rowContainer = useSelector(
    (state: RootState) => state.keplerGl[mapId]?.visState?.datasets?.choro_data?.dataContainer
  );
